...
def extract_info_from_filename(filename):
    ...
    match = re.search(
        r"([^_]+)_([^_]+)_beams_(\d+)_k_(\d+)_coco_expand_ratio_([\d.]+)_seed_(\d+)_max_tokens_(\d+)_samples_(\d+)_chair",
        filename,
    )
    if match:
        return match.groups()
...
with open(csv_file_path, "w", newline="") as csvfile:
    ...
    for file_name in sorted_file_names:
        if file_name.endswith("_chair.json"):
            ...
            print(
                f"| {backbone} | {decoder} | {expand_ratio} | {beam_size} | {k_number} | {seed_number} | {' | '.join(metrics)} | {max_tokens} | {num_samples} |\n"
            )
            if metrics:
